...
describe('GameActionFeedService', () => {
  ...
  it('should do nothing and clear state if publicState is null', () => {
  ...
  it('should initialize on first state emit without queuing existing history', () => {
  ...
  it('should parse and queue new history entries', () => {
      // 1. Initial State
      ...
      jest.advanceTimersByTime(200); // give it a moment to showNext
      expect(service.currentAction()?.id).toBe('C'); 
  ...
  it('should dismissCurrent when called and advance queue', () => {
      // Setup initial
  ...
  it('should resolve different durations', () => {
       mockSocketGame.publicState.set({ history: [] });
       TestBed.flushEffects();
       jest.advanceTimersByTime(1);
